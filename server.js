 ...
 */
/**
 * Helper: Resolve occupation term to URI
 */
async function resolveOccupationToUri(occupationName) {
  try {
    const [rows] = await ragPool.execute(`
      SELECT occupation_uri as uri, pref_label
      FROM occupation_labels
      WHERE LOWER(label) = ? OR LOWER(pref_label) = ?
      ORDER BY
        CASE WHEN LOWER(label) = ? THEN 0 ELSE 1 END,
        CASE WHEN occupation_uri LIKE '%/uwv/%' THEN 0 ELSE 1 END
      LIMIT 1
    `, [occupationName.toLowerCase(), occupationName.toLowerCase(), occupationName.toLowerCase()]);

    if (rows.length > 0 && rows[0].uri) {
      console.log(`[Generate] ✓ Resolved "${occupationName}" → "${rows[0].pref_label}" (${rows[0].uri})`);
      return rows[0].uri;
    }

    console.log(`[Generate] ⚠️  No URI found for "${occupationName}"`);
    return null;
  } catch (error) {
  ...
  // SCENARIO 4: Beroep vaardigheden (met resolved concept)
    ...
    if (Object.keys(resolvedConcepts).length > 0) {
      ...
      if (occupationMatch) {
        ...
        const resolvedUri = await resolveOccupationToUri(occName);
        if (resolvedUri) {
          occupationFilter = `VALUES ?occupation { <${resolvedUri}> }`;
        } else {
