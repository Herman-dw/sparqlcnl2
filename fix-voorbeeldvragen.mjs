...
async function main() {
  ...
  try {
    console.log('[1/6] Verbinden met database...');
    connection = await mysql.createConnection(dbConfig);
    console.log('      ✓ Verbonden met', dbConfig.database);

    // Stap 2: Fix de tabelstructuur EERST
    console.log('[2/6] Tabelstructuur fixen...');
    
    // Maak embedding kolom nullable
    try {
      await connection.execute(`
        ALTER TABLE question_embeddings 
        MODIFY COLUMN embedding LONGBLOB NULL DEFAULT NULL
      `);
      console.log('      ✓ embedding kolom is nu optioneel');
    } catch (e) {
      console.log('      → embedding:', e.message.substring(0, 60));
    }
    ...
    try {
      ...
      try {
        ...
        console.log('      → domain:', e2.message.substring(0, 60));
      }
    }

    // Stap 3: Bekijk huidige tabelstructuur
    console.log('[3/6] Tabelstructuur controleren...');
    const [columns] = await connection.execute(`
      SELECT COLUMN_NAME, IS_NULLABLE, COLUMN_DEFAULT 
      FROM INFORMATION_SCHEMA.COLUMNS 
      WHERE TABLE_SCHEMA = ? AND TABLE_NAME = 'question_embeddings'
    `, [dbConfig.database]);
    
    for (const col of columns) {
      if (['embedding', 'domain', 'question', 'category'].includes(col.COLUMN_NAME)) {
        console.log(`      - ${col.COLUMN_NAME}: nullable=${col.IS_NULLABLE}, default=${col.COLUMN_DEFAULT || 'none'}`);
      }
    }

    // Stap 4: Leeg de tabel
    console.log('[4/6] Bestaande voorbeelden verwijderen...');
    await connection.execute('DELETE FROM question_embeddings');
    console.log('      ✓ Tabel geleegd');

    // Stap 5: Voeg nieuwe voorbeelden toe
    console.log('[5/6] Nieuwe voorbeeldvragen toevoegen...');
    
    let successCount = 0;
    for (const vraag of VOORBEELDVRAGEN) {
      try {
        await connection.execute(
          `INSERT INTO question_embeddings (question, sparql_query, category, domain) 
           VALUES (?, ?, ?, ?)`,
          [vraag.question, vraag.sparql_query, vraag.category, vraag.domain]
        );
        console.log(`      ✓ "${vraag.question.substring(0, 35)}..."`);
        successCount++;
      } catch (insertError) {
        console.log(`      ✗ FOUT bij "${vraag.question.substring(0, 25)}": ${insertError.message}`);
      }
    }

    // Stap 6: Controleer resultaat
    console.log('[6/6] Resultaat controleren...');
    ...
    if (count > 0) {
      ...
      console.log('============================================');
      console.log('  ✗ Geen vragen toegevoegd');
      console.log('============================================');
    }
    console.log('');
