    ...
    <div className="relative" ref={dropdownRef}>
      <div className="relative">
        ...
        <input
          ...
          disabled={disabled}
          className="w-full pl-10 pr-10 py-3 border border-slate-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:border-transparent disabled:bg-slate-50 disabled:text-slate-400"
        />
        {isSearching && (
      ...
      {showDropdown && results.length > 0 && (
        <div className="absolute z-50 w-full mt-1 bg-white border border-slate-200 rounded-xl shadow-lg max-h-60 overflow-y-auto">
          {results.map((item, idx) => (
            <button
              key={item.uri || idx}
              className="w-full text-left px-3 py-2 hover:bg-emerald-50 focus:bg-emerald-50 focus:outline-none flex flex-col gap-1"
