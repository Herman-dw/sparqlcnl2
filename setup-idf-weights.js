...
// Input bestand
const IDF_JSON_FILE = path.join(__dirname, "idf-weights.json");
...
async function main() {
  ...
  if (!fs.existsSync(IDF_JSON_FILE)) {
    console.error(`   ❌ Bestand niet gevonden: ${IDF_JSON_FILE}`);
    console.error("   Zorg dat idf-weights.json in dezelfde map staat als dit script.");
    process.exit(1);
  }
